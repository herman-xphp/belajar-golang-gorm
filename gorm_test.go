...
func TestExecSQL(t *testing.T) {
	...
	err = db.Exec("insert into sample(id, name) values (?,?)", "2", "Anto").Error
	assert.Nil(t, err)

	err = db.Exec("insert into sample(id, name) values (?,?)", "3", "Wahyu").Error
	assert.Nil(t, err)

	err = db.Exec("insert into sample(id, name) values (?,?)", "4", "Arif").Error
	assert.Nil(t, err)
}
...
func TestUpdate(t *testing.T) {
	user := User{}
	err := db.Take(&user, "id = ?", "1").Find(&user).Error
	assert.Nil(t, err)
...
func TestUpdateSelectedColumns(t *testing.T) {
	...
	err = db.Model(&User{}).Where("id = ?", "1").Update("password", "apaya?").Error
	assert.Nil(t, err)
...
func TestAssociationDelete(t *testing.T) {
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		var product Product
		err = tx.Take(&product, "id = ?", "P001").Error
		assert.Nil(t, err)

		err = tx.Model(&product).Association("LikedByUsers").Delete(&user)
		return err
	})
...
func TestAssociationClear(t *testing.T) {
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		tx.Model(&product).Association("LikedByUsers").Clear()
		return err
	})
	assert.Nil(t, err)
}
